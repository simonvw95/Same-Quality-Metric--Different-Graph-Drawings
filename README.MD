...
### Minimal example
With a single git clone (make sure you use git lfs to properly clone the csv files (in /data/) or dowload the csv files manually) you can start fooling metrics on any target shape and graph.
...
1. Before you try to start fooling the graph drawings, ensure the following are correct.
...
- Adjust **main_mult.py** to reduce or increase the number of graphs/target shapes/metrics. (Set the variable name_parts to None incase you do not want to generate a .json file for every experiment, the .json file records the coordinates and qms every 10 iterations)
...
3. Once the algorithm has generated new coordinates close to the target shapes for your input graphs, run **final_drawings.py** to produce .pdf images of your resulting fooled drawings.
...
### Deprecated scripts
**final_sim_values**: Script to produce .pdf files containing the similarity percentages. Replaced by jitterplot_stat_test.py.
...
## Example Figures of Fooled Drawings
For the following six target shapes we fool 5 graphs ('bar-albert, 'polbooks', 'lnsp_131', 'gams10am', 'dwt_307') on four different quality metrics (ST : Stress, ELD : Edge Length Deviation, CN : number of crossings, AR : Angular Resolution).
![](example_figures/targetshapes_github.png)
![](example_figures/teaser.png)
![](example_figures/main_paper_results.png)

For videos of how these drawings slowly morph into their targets see the .mp4 files in results/final_drawings/
...
<img src="example_figures/rr-ba_rr-ST.gif" width="425"/> 
<img src="example_figures/rr-ba_rr-ELD.gif" width="425"/>
<img src="example_figures/rr-ba_rr-CN.gif" width="425"/>
<img src="example_figures/rr-ba_rr-AR.gif" width="425"/>


Keeping all 4 metrics exactly the same at the same time is more difficult:

<img src="example_figures/rr-ba_rr-ST-ELD-CN-AR.gif" width="425"/>
